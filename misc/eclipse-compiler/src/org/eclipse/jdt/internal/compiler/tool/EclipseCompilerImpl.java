    ...
    if (diagListener != null) {
     diagListener.report(new Diagnostic<JavaFileObject>() {
      ...
      public JavaFileObject getSource() {
       // BEGIN BUGFIX by MGR: Bug 308256 – DiagnosticListener always supplies Diagnostic.getSource()==null 
       // https://bugs.eclipse.org/bugs/show_bug.cgi?id=308256
       if (EclipseCompilerImpl.this.fileManager instanceof StandardJavaFileManager) {
        StandardJavaFileManager fm = (StandardJavaFileManager)EclipseCompilerImpl.this.fileManager;
        Iterable<? extends JavaFileObject> jfos = fm.getJavaFileObjects(new String(originatingFileName));
        java.util.Iterator<? extends JavaFileObject> jfoIt = jfos.iterator();
        if (jfoIt.hasNext()) return jfoIt.next();
        return null;
       }
       // END BUGFIX by MGR: Bug 308256 – DiagnosticListener always supplies Diagnostic.getSource()==null 
       // https://bugs.eclipse.org/bugs/show_bug.cgi?id=308256
       try {
      ...
      public JavaFileObject getSource() {
       ...
       try {
        if (EclipseCompilerImpl.this.fileManager.hasLocation(StandardLocation.SOURCE_PATH)) {
         return EclipseCompilerImpl.this.fileManager.getJavaFileForInput(
           StandardLocation.SOURCE_PATH,
           "Foo",//new String(originatingFileName),
           JavaFileObject.Kind.SOURCE);
        }
       } catch (IOException e) {
        // ignore
