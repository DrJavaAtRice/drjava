...
public class FindReplaceTest extends TestCase {
  ...
  public void testReplace() {
    try {
      _defPane.getDocument().insertString(0, "Tell all your friends.  I am Batman.", 
          null);
    } catch (BadLocationException e) {}
  ...
  public void testReplaceFindNext() {
    try {
      _defPane.getDocument().insertString(0, "To be or not to be, that is the question.", 
          null);
    } catch (BadLocationException e) {}
    assertTrue("#0.0", _defPane.findNextTextHelper("be", false, true));
    assertEquals("#0.1", "be", _defPane.getSelectedText());
    assertTrue("#1.0", _defPane.replaceFindTextHelper("be", "rock", false, true));
    assertEquals("#1.1", 18, _defPane.getSelectionStart());
    assertEquals("#1.2", 20, _defPane.getSelectionEnd());
    assertTrue("#2.0", !_defPane.replaceFindTextHelper("be", "rock", false, true));
    int length = _defPane.getDocument().getLength();
    try {
      assertEquals("#2.1", "To rock or not to rock, that is the question.", _defPane.getDocument().getText(0, 
          length));
    ...
    assertTrue("#4.1", _defPane.replaceFindTextHelper("rock", "be", false, true));
    assertEquals("#4.2", 7, _defPane.getCaretPosition());
    assertTrue("#4.3", !_defPane.replaceFindTextHelper("rock", "be", false, true));
    length = _defPane.getDocument().getLength();
    try {
      assertEquals("#4.4", "To be or not to be, that is the question.", _defPane.getDocument().getText(0, 
          length));
    } catch (BadLocationException f) {}
    // confirm = no
    _defPane.setCaretPosition(15);
    assertTrue("#5.0", _defPane.findNextTextHelper("be", false, false));
    assertTrue("#5.1", !_defPane.replaceFindTextHelper("be", "rock", false, false));
    length = _defPane.getDocument().getLength();
    try {
      assertEquals("#5.2", "To be or not to rock, that is the question.", _defPane.getDocument().getText(0, 
          length));
    } catch (BadLocationException f) {}
  }
  ...
  public void testReplaceAll() {
    try {
      _defPane.getDocument().insertString(0, "First shalt thou take out the Holy Pin. Then shalt thou count "
          + "to three, no more, no less. Three shall be the number thou " + "shalt count, and the number of the counting shall be three. "
          + "Four shalt thou not count, neither count thou two, excepting " + "that thou then proceed to three. Five is right out. Once the "
