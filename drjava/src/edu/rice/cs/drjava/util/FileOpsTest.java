...
public class FileOpsTest extends TestCase {
  ...
  public void testReadAndWriteTempFile() throws IOException {
    File file = FileOps.writeToNewTempFile(PREFIX, SUFFIX, TEXT);
    try {
      ...
      String read = FileOps.readFile(file);
  ...
  public void testRecursiveDirectoryDelete() throws IOException {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      for (int j = 0; j < 2; j++) {
        ...
        FileOps.writeToFile(file, "Some text for file "+file.getAbsolutePath());
